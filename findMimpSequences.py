...
def mimpSearch(contig,sequence):
    foundHits = []
    posHits = re.compile(r'CAGT[GA][GA]G[GA][TG]GCAA.AAGT[TA]T.AAT.C[AC]').finditer(sequence)
    negHits = re.compile(r'[GT]G.ATT.A[TA]ACTT.TTGC[CA][TC]C[TC][TC]ACTG').finditer(sequence)
    for x in posHits:
