...
####Function process haploid variants 
#processHapVariants(){
#}
...
with open("$1", 'r') as fi:
	...
	for i in range(9, len(data[0])):
		...
		for x in data[1:]:
			if x[i][0] == "1" and x[i][2] == "1":
				variantDict[strain][" ".join(x[0:2] + x[3:5])] = [x[4] + " " + x[4]]
			elif  x[i][0] == "0" and x[i][2] == "1":
				variantDict[strain][" ".join(x[0:2] + x[3:5])] = [x[3] + " " + x[4]]
			else:
				pass
	...
	for x,y in variantDict.items():
		for y,z in y.items():
			j=" ".join(map(str,z))
		dataToProcess = []
		dataToProcess.append(" ".join([x,y,j]))

####################################################################################
with open("$2", 'r') as fii:
	gffData = [x.strip().split() for x in fii.readlines() if not x.startswith("#")]
	for x in gffData:
		region = x[2]
		start = x[3]
		end = x[4]
		if x[2] == "gene":
			gene=x[8].split("=")[1].split(";")[0]

		for x in dataToProcess:
			strain, contig, position, ref, alt, gt1, gt2 = x.strip().split()
...
if [ ${ploidy} -eq 1 ]; then
	...
	alleles=$(processDipVariants VCF GFF FASTA)
else
